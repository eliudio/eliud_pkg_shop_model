...
/* 
 * CartComponentSelector is a component selector for Cart, allowing to select a Cart component
 */
class CartComponentSelector extends ComponentSelector {
  /* 
  ...
  Widget createSelectWidget(BuildContext context, AppModel app,
    ...
    return BlocProvider<CartListBloc>(
      create: (context) => CartListBloc(
        ...
        cartRepository: cartRepository(appId: appId)!,
      )..add(LoadCartList()),
      child: _SelectCartWidget(
          app: app,
          height: height,
 ...
 * _SelectCartWidget 
 */
class _SelectCartWidget extends StatefulWidget {
  final AppModel app;
  final double height;
  ...
  @override
  State<_SelectCartWidget> createState() {
    return _SelectCartWidgetState();
  }
}

class _SelectCartWidgetState extends State<_SelectCartWidget>
  ...
  void _handlePrivilegeTabSelection() {
    if ((_privilegeTabController != null) &&
      ...
      BlocProvider.of<CartListBloc>(context).add(CartChangeQuery(
          newQuery: getComponentSelectorQuery(
              _currentPrivilege, widget.app.documentID)));
    }
  }

  Widget theList(BuildContext context, List<CartModel?> values) {
    var app = widget.app;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<CartListBloc, CartListState>(builder: (context, state) {
      ...
      if ((state is CartListLoaded) && (state.values != null)) {
